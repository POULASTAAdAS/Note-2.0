...
object Constants {
    ...
    const val BASE_URL = "https://1e63-103-192-118-176.ngrok-free.app"

    const val PREFERENCES_NAME = "appPreferences"

    const val PREFERENCES_SIGNED_IN_KEY = "preferencesSigningKey"
    const val PREFERENCES_JWT_TOKEN_OR_SESSION_TOKEN_KEY = "giveMeThatJwtTokenKeyOrCookieee"
    const val PREFERENCES_FIRST_TIME_SIGNED_IN_KEY = "preferencesFirstSigningKey"
    const val AUTH_TYPE_KEY = "authType"

    const val AUTO_SYNC_KEY = "autoSync"
    const val SORT_STATE_KEY = "sortState"
    const val NOTE_VIEW_KEY = "noteView"
